...
export const processRepository = async ({ name: projectName, repositoryUrl, root: inputRoot, output: outputRoot, llms, ignore, filePrompt, folderPrompt, contentType, targetAudience, linkHosted, }, dryRun) => {
    ...
    const processFile = async ({ fileName, filePath, projectName, contentType, filePrompt, targetAudience, linkHosted, }) => {
         ...
         */
        const reindex = await shouldReindex(path.join(outputRoot, filePath.substring(0, filePath.lastIndexOf('\\'))), fileName.replace(/\.[^/.]+$/, '.json'), newChecksum);
        if (!reindex) {
            return;
        }
        const markdownFilePath = path.join(outputRoot, filePath);
        const url = githubFileUrl(repositoryUrl, inputRoot, filePath, linkHosted);
        const summaryPrompt = createCodeFileSummary(projectName, projectName, content, contentType, filePrompt);
        const questionsPrompt = createCodeQuestions(projectName, projectName, content, contentType, targetAudience);
        ...
        const model = (() => {
            ...
            else if (models[LLMModels.GPT316k].maxLength > max &&
    ...
    const processFolder = async ({ folderName, folderPath, projectName, contentType, folderPrompt, shouldIgnore, linkHosted, }) => {
        /**
         ...
         * it will check the checksums and decide if a reindex is needed
         */
        const reindex = await shouldReindex(folderPath, 'summary.json', newChecksum);
        if (!reindex) {
            return;
        }
        ...
        try {
            ...
            const summary = await callLLM(folderSummaryPrompt(folderPath, projectName, files, folders, contentType, folderPrompt), models[LLMModels.GPT4].llm);
            const folderSummary = {
                folderName,
                folderPath,
                url,
 ...
 */
async function calculateChecksum(contents) {
    const checksums = [];
 ...
 */
async function shouldReindex(contentPath, name, newChecksum) {
