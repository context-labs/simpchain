...
export const query = async ({ name, repositoryUrl, output, contentType, chatPrompt, targetAudience }, { llms }) => {
    const data = path.join(output, 'docs', 'data/');
    const vectorStore = await HNSWLib.load(data, new OpenAIEmbeddings());
    const chain = makeChain(name, repositoryUrl, contentType, chatPrompt, targetAudience, vectorStore, llms, (token) => {
        stopSpinner();
